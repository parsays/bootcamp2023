...
class Basket:
    ...
    def add_products(self, basket: list, brand: str, model: str, quantity: int):
        for item in basket:
            if model in item:
                logger.warning(f'{self.username} wants to add {model} which added befpre')
    ...
    def search_store(self,model: str, store_list: list) -> str:
        similar_list=list()
        for item in store_list:
            score= self.similarity(item[2], model)
            if score >= 0.4:
                similar_list.append((item[2], score))
        similar_list=sorted(similar_list, key= lambda item: item[1], reverse=True)
        if similar_list:
            return similar_list
    ...
    def show_basket(self, basket: list):
        print("********YOUR BASKET********")
    ...
    def compare_products(self, store_list: list,store: list, models)-> list:
        print(models)
        gadgets= list()
        for item in models:
    ...
    def Product_pagination(self, page_number):
        results= list()
    ...
    def show_gadgets(self,finish_index, start_index, page, number_of_page, store_list: list):
        while True:
            try:
                for i in range(start_index, finish_index):
                    print(f'{i+1}. Brand: {store_list[i][1]}   Model: {store_list[i][2]}   Available: {store_list[i][3]["available"]}')
            ...
            try:
                ...
                print(f'You have to enter number in range 1 to {number_of_page} ')
            if page in EXIT_COMMANDS:
